...
def admin_session():
    ...
    data = r.json()
    assert data["email"] == ADMIN_EMAIL and data["role"] == "director"
...
# ---------- Levels ----------
class TestLevels:
    def test_create_and_list(self, admin_session, state):
        code = f"TEST-LVL-{uuid.uuid4().hex[:6].upper()}"
        payload = {"name": "TEST Beginner", "code": code, "monthly_fee": 1500}
        r = admin_session.post(f"{API}/levels", json=payload)
        ...
        r = admin_session.get(f"{API}/levels")
        assert r.status_code == 200
...
class TestBatches:
    def test_create_and_list(self, admin_session, state):
        payload = {"name": f"TEST Batch {uuid.uuid4().hex[:4]}", "level_id": state["level_id"],
                   "schedule_days": ["Mon", "Wed"], "session_time": "5pm", "max_capacity": 20}
        r = admin_session.post(f"{API}/batches", json=payload)
        assert r.status_code in (200, 201), r.text
        b = r.json()
        assert "id" in b
        state["batch_id"] = b["id"]

        r = admin_session.get(f"{API}/batches")
        assert r.status_code == 200
        rows = r.json()
...
# ---------- Students ----------
class TestStudents:
    def test_create_list_detail(self, admin_session, state):
        payload = {
            "full_name": f"TEST Student {uuid.uuid4().hex[:4]}",
            "parent_name": "TEST Parent",
            "parent_whatsapp": "+919999999999",
            "parent_email": f"test_{uuid.uuid4().hex[:6]}@example.com",
        ...
        r = admin_session.get(f"{API}/students/{s['id']}")
        assert r.status_code == 200
...
class TestAttendance:
    def test_save_get_upsert(self, admin_session, state):
        date = "2026-01-15"
        marks = {state["student_id"]: "P"}
        payload = {"batch_id": state["batch_id"], "session_date": date, "marks": marks}
        r = admin_session.post(f"{API}/attendance", json=payload)
        assert r.status_code in (200, 201), r.text

        r = admin_session.get(f"{API}/attendance",
                              params={"batch_id": state["batch_id"], "session_date": date})
        assert r.status_code == 200
        data = r.json()
...
# ---------- Invoices ----------
class TestInvoices:
    def test_create_list_pdf(self, admin_session, state):
        payload = {
            "student_id": state["student_id"],
            "period": "2026-02",
            "due_date": "2026-02-10",
            "items": [{"description": "Monthly Fee 2026-02", "amount": 1500}],
        }
        ...
        r = admin_session.get(f"{API}/invoices/{inv['id']}/pdf")
        assert r.status_code == 200
...
# ---------- Payments ----------
class TestPayments:
    def test_record_payment_and_receipt(self, admin_session, state):
        ...
        # Invoice now paid
        r = admin_session.get(f"{API}/invoices/{state['invoice_id']}")
        assert r.status_code == 200
        inv = r.json()
        ...
        # Receipt PDF
        if receipt_id:
            r = admin_session.get(f"{API}/receipts/{receipt_id}/pdf")
...
# ---------- Reminder (mock mode) ----------
class TestReminder:
    def test_remind_log_mode(self, admin_session, state):
        r = admin_session.post(f"{API}/invoices/{state['invoice_id']}/remind")
        assert r.status_code in (200, 201), r.text
        d = r.json()
...
# ---------- Settings & RBAC ----------
class TestSettings:
    def test_settings_academy(self, admin_session):
        r = admin_session.get(f"{API}/settings/academy")
        assert r.status_code == 200
        d = r.json()
    ...
    def test_create_user_director_only(self, admin_session):
        ...
        login = coach_session.post(f"{API}/auth/login", json={"email": payload["email"], "password": "Test@1234"})
